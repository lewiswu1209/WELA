...
class LLMAgent(Agent):
    ...
    def predict(self, **kwargs: Any) -> AIMessage | Generator:
        ...
        if not self._model.streaming:
            for i in range(self.__max_loop):
                if i == self.__max_loop - 1 or not self.__toolkit:
                    response_message = self._model.run(messages, stop=self.__stop)
                else:
                    response_message = self._model.run(messages, stop=self.__stop, tools=self.__toolkit.to_tools_param())
