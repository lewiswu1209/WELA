...
def __convert_time_format(time_str: str) -> str:
...
def __get_weather_forecast(
    city: Annotated[str, "The name of the city to get the weather for. This field is required and must be provided in English. Only one city is allowed."]
):
...
'''
                ...
                for hourly in each_day["hourly"]:
                    hourly_time = __convert_time_format(hourly["time"])
...
GetWeatherTool = FunctionTool(
    func=__get_weather_forecast,
