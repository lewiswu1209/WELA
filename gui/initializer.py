...
class Initializer(QObject):
    ...
    def initialize(self) -> None:
        ...
        toolkit = Toolkit([AlarmClock(), Quit(), Weather(), Definition(proxies), GoogleSearch(config.get("google_custom_search").get("api_key"), config.get("google_custom_search").get("search_engine_id"), proxies), WebBrowser(headless=False, proxy=proxy), WebBrowserScreenshot(model=tool_model, headless=False, proxy=proxy)], None)
